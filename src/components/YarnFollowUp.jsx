...
export default function YarnFollowUp({ showNotification }) {
  ...
  const fetchRecords = (currentPage = page, currentSearch = search, currentStatus = status) => {
    ...
    fetch(`/api/yarn?${query}`)
      ...
      .catch(err => {
        console.error(err);
        showNotification('Failed to fetch yarn records.', 'error');
        setLoading(false);
      });
  };
  ...
  const handleInputChange = (field, value) => {
    setFormData(prev => {
      ...
      if (field === "Demand Ton" || field === "Receive (Ton)") {
        const demand = parseFloat(updated["Demand Ton"] || 0);
        const receive = parseFloat(updated["Receive (Ton)"] || 0);
  ...
  const handleSaveModal = (e) => {
    ...
    const url = isEditMode ? `/api/yarn/${selectedRecord.RowNumber}` : '/api/yarn';
  ...
  const handleDelete = (rowNumber) => {
    requestPassword(() => {
      fetch(`/api/yarn/${rowNumber}`, { method: 'DELETE' })
        .then(res => {
  ...
  const renderStatusBadge = (item) => {
    const d = parseFloat(item["Demand Ton"] || 0);
    const r = parseFloat(item["Receive (Ton)"] || 0);
  ...
  const handleDownloadExcel = () => {
    ...
    window.open(`/api/yarn/download?${query}`, '_blank');
  };
      ...
      <div className="table-controls">
        <div className="search-input-wrapper">
          ...
          <input 
            ...
            placeholder="Search by Buyer, Supplier, Article, count..." 
            value={search}
      ...
      <div className="table-card">
        {loading ? (
          <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: '200px' }}>
            <p style={{ color: 'var(--text-secondary)' }}>Loading yarn follow-up database...</p>
          </div>
        ) : (
          <>
        ...
        <div className="modal-overlay">
          <div className="modal-window">
            <div className="modal-header">
              <h3 className="modal-title">{isEditMode ? `Edit Yarn Record (Row ${selectedRecord.RowNumber})` : 'New Yarn Record'}</h3>
              <button 
              ...
              <div className="modal-body">
                <div className="form-grid">
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                    ...
                    <div className="form-group">
                      <label className="form-label">Demanded Count (Yarn Count)</label>
                      <AutocompleteInput 
                        ...
                        placeholder="Search or enter yarn count..."
                        className="form-control"
                    ...
                    <div className="form-group">
                      <label className="form-label">Certification / Speciality</label>
                      <input 
                        type="text" className="form-control" 
                        value={formData["Certification / Speciality"] || ''} 
                        onChange={(e) => handleInputChange("Certification / Speciality", e.target.value)}
                      />
                    </div>
                  </div>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                    ...
                    <div className="form-group">
                      <label className="form-label">Yarn TC NO</label>
                      <input 
                        type="text" className="form-control" 
                        value={formData["Yarn TC NO"] || ''} 
                        onChange={(e) => handleInputChange("Yarn TC NO", e.target.value)}
                      />
                    </div>
                    <div className="form-group">
                      ...
                      <input 
                        type="text" className="form-control" 
                        value={formData["TC Status"] || ''} 
                        onChange={(e) => handleInputChange("TC Status", e.target.value)}
                      />
                    </div>
                    <div className="form-group">
                      <label className="form-label">Demand (Tons)</label>
                      <input 
                        type="number" step="any" className="form-control" 
                        value={formData["Demand Ton"] || '0'} 
                        onChange={(e) => handleInputChange("Demand Ton", e.target.value)}
                      />
                    </div>
                    <div className="form-group">
                      <label className="form-label">Receive (Tons)</label>
                      <input 
                        type="number" step="any" className="form-control" 
                        value={formData["Receive (Ton)"] || '0'} 
                        onChange={(e) => handleInputChange("Receive (Ton)", e.target.value)}
                      />
                    </div>
                    <div className="form-group">
                    ...
                    <div className="form-group">
                      ...
                      <input 
                        ...
                        onChange={(e) => handleInputChange("Procurement", e.target.value)}
                      />
                    </div>
                    <div className="form-group">
